view: zds_vs_b2m {
  derived_table: {
    sql: SELECT
      ...
      WHERE
        _et > TIMESTAMP(DATE_SUB(CURRENT_DATE(), INTERVAL 75 DAY))
        AND DATE(TIMESTAMP_MILLIS(CAST(_r AS INT64))) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 75 DAY)
      ...
      WHERE
        _et > TIMESTAMP(DATE_SUB(CURRENT_DATE(), INTERVAL 75 DAY))
        AND DATE(TIMESTAMP_MILLIS(CAST(raised AS INT64))) BETWEEN DATE_SUB(CURRENT_DATE(), INTERVAL 75 DAY)
        AND DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)
      GROUP BY
        raised_date
      ORDER BY
